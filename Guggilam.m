function [V, Pc, Qc, Gug_V, Gug_I2R] = Guggilam(testCase, T, solarHH, loadHH, multiPer, per);
% inputs
...
if multiPer == 0
    ...
    for k = 1% : load_scenarios
        ...
        % update solar data
        if nPV ~= 0
             Pav(idxPV-1) = mpc.gen(2:end,9)*solarHH(per)/baseMVA; % Pmax
             Sinj(idxPV-1) = mpc.gen(2:end,9)*solarHH(per)*inverterSize/baseMVA; % Pmin
        end
        % update load data
...
elseif multiPer == 1
    ...
    for t = 1 : T
        ...
        % update solar data
        if nPV ~= 0
            Pav(idxPV-1) = mpc.gen(2:end,9)*solarHH(t)/baseMVA; % Pmax
            Sinj(idxPV-1) = mpc.gen(2:end,9)*solarHH(t)*inverterSize/baseMVA; % Pmin
        end
        % update load data
